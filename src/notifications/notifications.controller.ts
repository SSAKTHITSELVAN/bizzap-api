...
export class NotificationsController {
  ...
  async registerToken(
    ...
    // ✅ FIX: Matched method name with Service (registerToken)
  ...
  @ApiOperation({ summary: 'Unregister Expo push token' })
  async unregisterToken(@Body('token') token: string) {
    // ✅ FIX: Matched method name with Service (unregisterToken)
  ...
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'Mark notifications as read' })
  async markAsRead(@Body() dto: MarkNotificationReadDto) {
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'Send broadcast (Admin)' })
  async sendBroadcast(@Body() broadcastDto: SendBroadcastDto) {
    await this.notificationsService.sendBroadcastNotification(
      broadcastDto.title,
      broadcastDto.body,
