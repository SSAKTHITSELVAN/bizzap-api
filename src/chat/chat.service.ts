...
export class ChatService {
  ...
  async getChatHistory(companyId: string, otherCompanyId: string): Promise<any[]> {
    ...
    // Generate signed URLs for files and company assets
  ...
  async getAllConversations(companyId: string): Promise<any[]> {
    ...
    // Convert to array, add unread counts, and generate signed URLs
    const conversations: any[] = [];
    for (const [partnerId, conv] of conversationMap) {
      const unreadCount = await this.getUnreadCount(companyId, partnerId);
      
      // Generate signed URLs for partner's logo
      let partnerLogo: string | null = conv.partner.logo || null;
      ...
      // Generate signed URLs for last message file/thumbnail (if applicable)
      let lastMessageFileUrl: string | null = null;
      let lastMessageThumbnailUrl: string | null = null;
  ...
  /**
   * 🆕 Helper method to generate signed URLs for messages and company assets
   * Handles: fileUrl, thumbnailUrl, sender.logo, receiver.logo
   */
  private async generateSignedUrlsForMessages(messages: Chat[]): Promise<any[]> {
    return Promise.all(
      messages.map(async (message) => {
        const messageObj = { ...message };
        ...
        // Generate signed URLs for sender's company assets
        if (messageObj.sender) {
          await this.generateSignedUrlsForCompany(messageObj.sender);
        }

        // Generate signed URLs for receiver's company assets
  ...
  /**
   * 🆕 Helper method to generate signed URLs for company assets
   * Handles: logo, userPhoto, coverImage
   */
