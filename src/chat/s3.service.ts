...
// src/chat/services/s3.service.ts - FIXED VERSION
import { Injectable } from '@nestjs/common';
import { ConfigService } from '@nestjs/config';
import { S3 } from 'aws-sdk';
import { v4 as uuidv4 } from 'uuid';
...
export enum AssetType {
  ...
  COVER_IMAGE = 'cover-images',
  CHAT_FILE = 'chat-files',
  PRODUCT_IMAGE = 'product-images', // 🆕 Added
}
...
export class S3Service {
  ...
  /**
   * 🔧 FIXED: Check if a string is an S3 key (not a full URL)
   * S3 keys follow pattern: folder/uuid.extension
   */
  isS3Key(path: string): boolean {
    if (!path) return false;
    ...
    // 🔧 FIXED: Added 'product-images' to the pattern
